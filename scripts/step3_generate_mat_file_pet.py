...
path_save = "./media/ADNI1_baseline_Processed_AD_Input/PET"

files = os.listdir(path)
for fi in files:

    path_son = os.path.join(path, fi)
    fPET = os.path.join(path_son, 'pet_registration_better.nii.gz')
    if os.path.exists(fPET):
        
        img_PET = nib.load(fPET).get_fdata()
        img3d_PET = img_PET[10:163, 16:204, 10:163] 

        level = img3d_PET.shape 
